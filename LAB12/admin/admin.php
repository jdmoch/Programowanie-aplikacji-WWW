...
function DodajProdukt() {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $query = "INSERT INTO produkty 
                  (tytul, opis, data_utworzenia, data_wygasniecia, cena_netto, podatek_vat, ilosc_magazyn, status_dostepnosci, kategoria, gabaryt_produktu, zdjecie)
                  VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        $stmt = $link->prepare($query);
...
function EdytujProdukt() {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $query = "UPDATE produkty SET 
                  tytul = ?, opis = ?, data_modyfikacji = ?, data_wygasniecia = ?, cena_netto = ?, podatek_vat = ?, ilosc_magazyn = ?, status_dostepnosci = ?, kategoria = ?, gabaryt_produktu = ?, zdjecie = ?
                  WHERE id = ?";
        $stmt = $link->prepare($query);
    ...
    $query = "SELECT tytul, opis, data_wygasniecia, cena_netto, podatek_vat, ilosc_magazyn, status_dostepnosci, kategoria, gabaryt_produktu, zdjecie FROM produkty WHERE id = ?";
    $stmt = $link->prepare($query);
    $stmt->bind_param('i', $id);
    $stmt->execute();
    $result = $stmt->get_result();
    ...
    echo 'Data wygaśnięcia: <input type="date" name="data_wygasniecia" value="' . $row['data_wygasniecia'] . '"><br><br>';
